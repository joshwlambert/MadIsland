...
data("madagascar_amphibians_dna", package = "MadIsland")

# check that the data has loaded correctly and that it has the correct data
head(madagascar_amphibians_dna)
all(
  colnames(madagascar_amphibians_dna) ==
...
endemicity_status_dna <- lapply(
  ...
  island_species = madagascar_amphibians_dna
)
