...
## Collating DAISIE results {.tabset}

This step is dependent on whether in the previous step the raw DAISIE output has been commited to version control (e.g. git/GitHub). If the files have been committed and pushed to GitHub then the raw DAISIE output can be pulled onto a local machine and collated locally. If not then the collation will need to take place on the HPCC. 

The benefit of doing this step on the HPCC and not locally is there a many raw DAISIE output files and storing these on version control can increase the size of the git history of the respository.

### Local

Run the scripts in `scripts/collate_daisie_results/`.

The collated results are saved as `.rds` and can be pushed to version control.

### HPCC

Run the scripts in `scripts/collate_daisie_results/`. The code from these scripts can be run in the R console by loading and opening R (`ml R` and `R`) on the HPCC. Alternatively these scripts can be run with the `Rscript` command. 
...
Once collated DAISIE data is saved in the package, the plotting scripts in `inst/scripts/plots/` can be run to produce the plots in `inst/plots/`.

Some plots in the manuscript are post-processed to add silhouette (from [phylopic](https://www.phylopic.org/)). The SVGs of the plots that get modified are saved and used for editing, which are then saved as PNGs after post processing. The SVGs are added to the `.gitignore` file so will not be commited to git, and can optionally be deleted. Only the PNGs which are used in the manuscript are stored by the git version control.
