...
squamate_posterior_dna <- ape::read.nexus(
  file = system.file(
    "extdata/Tonini_dna_posterior_100.nex",
    package = "MadIsland"
  )
)
...
saveRDS(
  ...
  file = "inst/extdata/squamate_island_tbl_dna.rds"
)

# 2) the DAISIE data table
saveRDS(
  object = daisie_datatable_dna,
  file = "inst/extdata/squamate_daisie_datatable_dna.rds"
)

# 3) the DAISIE data list
saveRDS(
  object = daisie_data_list_dna,
  file = "inst/extdata/squamate_daisie_data_list_dna.rds"
)
