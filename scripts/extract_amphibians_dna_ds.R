...
data("madagascar_amphibians_dna_ds", package = "MadIsland")

# check that the data has loaded correctly and that it has the correct data
head(madagascar_amphibians_dna_ds)
all(
  colnames(madagascar_amphibians_dna_ds) ==
...
#delete
amphibian_posterior_dna <- amphibian_posterior_dna[1:3]

# convert trees to phylo4 objects
dna_phylos <- lapply(amphibian_posterior_dna, phylobase::phylo4)

# remove phylo objects to free up some memory
rm(amphibian_posterior_dna)
gc()
...
endemicity_status_dna <- lapply(
  ...
  island_species = madagascar_amphibians_dna_ds
)
...
no_phylo_missing_amphibian_species_dna <- lapply(
  missing_genus_dna,
...
sapply(
  list(
    no_phylo_missing_amphibian_species_dna[[2]]$clade_name,
    no_phylo_missing_amphibian_species_dna[[3]]$clade_name
  ), identical,
...
saveRDS(
  ...
  file = file.path(
    ...
    "extdata",
    "extracted_data",
    "amphibian_island_tbl_dna_ds.rds"
  )
)
...
saveRDS(
  ...
  file = file.path(
    ...
    "extdata",
    "extracted_data",
    "amphibian_daisie_datatable_dna_ds.rds"
  )
)
...
saveRDS(
  ...
  file = file.path(
    ...
    "extdata",
    "extracted_data",
    "amphibian_daisie_data_list_dna_ds.rds"
  )
)
